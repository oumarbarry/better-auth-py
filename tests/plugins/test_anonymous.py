...
async def test_cleanup_clears_anon_secondary_storage_sessions():
    """TS a03e4c186 dropped the plugin's own `deleteUserSessions` call
    (plugins/anonymous/index.ts:342) — `deleteUser` now owns the KV sweep too, so the
    post-link cleanup must still empty secondary storage on its own."""
    ss = MemorySecondaryStorage()
    auth = make_auth(plugins=[AnonymousPlugin()], secondary_storage=ss)
    anon_user, ctx = await _seed_anon_user_and_ctx(auth, path="/sign-in/email")
    ctx.new_session = {
        "session": {"token": "linked-token"},
        "user": {"id": "linked-user", "isAnonymous": False},
    ...
    assert await auth.adapter.find_one("user", [Where("id", anon_user["id"])]) is None
    assert await ss.get(f"active-sessions-{anon_user['id']}") is None
