...
Mirrors better-auth's plugins/haveibeenpwned/haveibeenpwned.test.ts. TS source verified
against:
  packages/better-auth/src/plugins/haveibeenpwned/index.ts

The plugin registers a check into ``auth.password_checks``; most cases call that check
callable directly with ``(password, path)`` -- it's the whole contract, and doing so lets
us exercise every default `paths` entry (including ones for plugins like admin and
phone-number, which this test module doesn't register) without needing those routes to
exist on this instance. Two tests go through a real sign-up/change-password HTTP round
trip via ``conftest.make_client`` to prove end-to-end wiring.
"""
