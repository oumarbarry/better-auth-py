...
async def test_list_device_sessions_hook_maps_fn_over_each_entry():
    # ponytail: exercised directly against the after-hook here (unit-level) rather
    # than wiring MultiSessionPlugin through real HTTP dispatch; matches the shape
    # `/multi-session/list-device-sessions` actually returns.
    plugin = CustomSessionPlugin(_augment, should_mutate_list_device_sessions_endpoint=True)
    auth = make_auth()
