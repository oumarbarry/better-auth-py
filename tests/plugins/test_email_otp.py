...
# "email-verification" it actually sends. The port's `_after_sign_up` (email_otp.py) has
# always passed the literal "email-verification" to `_generate` — never derived it from
# the request body — so this fix's premise doesn't hold here; N/A, proven below on
# unmodified source (magic-link precedent).


async def test_generate_otp_receives_email_verification_type_on_sign_up():
    seen: list[dict[str, Any]] = []

    def generate_otp(data: dict[str, Any], ctx: Any) -> str:
        seen.append(data)
        return "123456"
    ...
    async with make_client(auth) as client:
        await signup(client, "gentype@example.com", "password123")
        assert seen == [{"email": "gentype@example.com", "type": "email-verification"}]
        assert box["otp"] == "123456"
...
async def test_reset_password_preserves_otp_after_password_too_short():
    """TS 2b4a14f18 — a too-short password must not burn the OTP; the same code retries."""
    auth, box, _ = otp_auth()
    async with make_client(auth) as client:
        await signup(client, "ada@example.com", "orig-password")
        await send_otp(client, "ada@example.com", "forget-password")
        otp = box["otp"]
        invalid = await reset_pw(client, "ada@example.com", otp, "short")
        assert invalid.status_code == 400
        assert invalid.json()["code"] == "PASSWORD_TOO_SHORT"
        valid = await reset_pw(client, "ada@example.com", otp, "valid-password")
        assert valid.status_code == 200, valid.text
        assert valid.json()["success"] is True


async def test_reset_password_preserves_otp_after_password_too_long():
    auth, box, _ = otp_auth()
    async with make_client(auth) as client:
        await signup(client, "ada@example.com", "orig-password")
        await send_otp(client, "ada@example.com", "forget-password")
        otp = box["otp"]
        invalid = await reset_pw(client, "ada@example.com", otp, "a" * 129)
        assert invalid.status_code == 400
        assert invalid.json()["code"] == "PASSWORD_TOO_LONG"
        valid = await reset_pw(client, "ada@example.com", otp, "valid-password")
        assert valid.status_code == 200, valid.text
        assert valid.json()["success"] is True
...
async def test_check_no_row_returns_invalid_otp_not_user_not_found():
    """TS d8327f1fe reorders the check: the OTP row lookup now runs BEFORE the
    user-existence check, so a nonexistent email (no verification row, since
    send-verification-otp drops the row rather than leak the account) reports the
    same INVALID_OTP as any other unrecognized code — not USER_NOT_FOUND."""
    auth, _, _ = otp_auth()
    async with make_client(auth) as client:
...
async def test_check_otp_enumeration_guard_same_error_registered_and_unregistered():
    """TS d8327f1fe (https://github.com/better-auth/better-auth/issues/10603): the OTP is
    verified before the account's existence is revealed, so a wrong/missing OTP yields
    INVALID_OTP identically for a registered and an unregistered email — the previous
    order (user check first) let an attacker distinguish the two via the error code."""
    auth, _, _ = otp_auth()
    async with make_client(auth) as client:
        await signup(client, "existing@domain.com")
        registered = await check_otp(client, "existing@domain.com", "email-verification", "000000")
        unregistered = await check_otp(
            client, "non-existent@domain.com", "email-verification", "000000"
        )
        assert registered.status_code == unregistered.status_code == 400
        assert registered.json()["code"] == unregistered.json()["code"] == "INVALID_OTP"
