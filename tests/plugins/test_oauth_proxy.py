...
async def start_sign_in(
    client: AsyncClient, callback_url: str = "/dashboard", provider: str = "github"
) -> str:
...
def apple_http(id_token: str) -> httpx.AsyncClient:
    def handler(request: httpx.Request) -> httpx.Response:
...
def make_apple_instance(
    base_url: str, plugin: OAuthProxyPlugin, id_token: str = "", **overrides: Any
) -> BetterAuth:
    return BetterAuth(
        secret=overrides.pop("secret", SECRET),
        base_url=base_url,
...
async def test_production_callback_preserves_apple_user_data():
    """TS e7b0eba32 (https://github.com/better-auth/better-auth/issues/10598): Apple's id
    token carries no ``name`` claim — the real name arrives only once, via the ``user``
    field of its ``form_post`` callback. The production before-callback hook must merge
    that field into the encrypted profile it replays to the preview, not drop it."""
    apple_id_token = jwt.encode(
        {
            "sub": "apple-user-id",
    ...
    preview = make_apple_instance(
        "http://preview.example.com",
        OAuthProxyPlugin(production_url="http://production.example.com"),
        id_token=apple_id_token,
    )
    production = make_apple_instance(
        "http://production.example.com", OAuthProxyPlugin(), id_token=apple_id_token
    )
    async with make_client(preview) as pv, make_client(production) as prod:
        state = state_of(await start_sign_in(pv, "/dashboard", provider="apple"))
        user_data = json.dumps({"name": {"firstName": "Jane", "lastName": "Doe"}})
        response = await prod.post(
        ...
        assert payload["userInfo"]["name"] == "Jane Doe"
        assert payload["userInfo"]["email"] == "jane@privaterelay.appleid.com"
