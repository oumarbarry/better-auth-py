...
@pytest.fixture
def auth() -> BetterAuth:
    # Litestar warns on a test host without a dot; keep base_url, host and origin aligned.
    return make_auth(base_url="http://testserver.local")
...
def make_client(auth: BetterAuth) -> AsyncTestClient:
    ...
    client: AsyncTestClient = AsyncTestClient(
        app=make_app(auth), base_url="http://testserver.local"
    )
    client.headers.update({"origin": "http://testserver.local"})
    return client
...
async def test_custom_base_path_mount():
    auth = make_auth(base_url="http://testserver.local", base_path="/auth")
    async with make_client(auth) as client:
