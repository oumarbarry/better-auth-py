...
async def verify_password_handler(ctx: Ctx) -> AuthResponse:
    ...
    not client-callable) but still routes it through the shared HTTP handler;
    this port does the same. Wire shape matches TS exactly: ``{status:true}``
    on success, throws ``INVALID_PASSWORD`` (never ``{valid:false}``) on
    mismatch.
    """
    result = await ctx.require_session()
    body = ctx.body()
...
async def _verify_change_email(
    ...
    ponytail: `afterEmailVerification` hook is not called here: `EmailVerification`
    has no such field yet (only `send_verification_email` and the boolean flags);
    wire it in when that config grows the callback.
    """
    cfg = ctx.auth.email_verification
