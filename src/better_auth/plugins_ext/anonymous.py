...
class AnonymousPlugin(Plugin):
    ...
    async def _after_link_or_verify(self, ctx: Ctx) -> None:
        ...
        if self.disable_delete_anonymous_user or is_same_user or new_session_is_anonymous:
            return None
        # TS a03e4c186 (plugins/anonymous/index.ts:342): no separate delete_user_sessions
        # — delete_user owns session cleanup (KV included), so there is no window where
        # sessions are gone but the user row survives. TS logs and swallows a failure:
        # best-effort post-link cleanup, not fatal to the response that already succeeded.
