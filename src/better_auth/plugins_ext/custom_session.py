...
class CustomSessionPlugin(Plugin):
    ...
    def _matches_list_device_sessions(self, ctx: Ctx) -> bool:
        # ponytail: matched by the literal path string instead of importing
        # `MultiSessionPlugin`, so this fires whenever any registered plugin serves
        # that path and the integrator opts in via
