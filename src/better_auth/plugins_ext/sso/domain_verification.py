...
- ``POST /sso/verify-domain`` — for every domain in ``parseProviderDomains(provider.domain)``
  ...
  all-or-nothing (any miss -> 502). On full success flip ``domainVerified`` through a
  compare-and-swap on the provider row (409 ``SSO_PROVIDER_CHANGED`` if it moved under the
  in-flight lookup); status 204.
...
async def verify_domain(plugin: SSOPlugin, ctx: Ctx) -> AuthResponse:
    ...
    await _complete_domain_verification(plugin, ctx, provider)
    return AuthResponse(status=204)


async def _complete_domain_verification(plugin: SSOPlugin, ctx: Ctx, provider: dict) -> None:
    """Flip ``domainVerified`` with a compare-and-swap on the provider row (999acbd41).

    Guarding on ``id`` + ``domain`` is what binds the proof to what DNS answered for: a
    provider mutated, replaced or deleted while the lookup was in flight no longer matches,
    so the stale proof is refused instead of verifying a domain nobody proved.
    ``domainVerified`` stays out of the guard — it is nullable for providers that predate
    the option, and an equality guard would never match those."""
    verified = await ctx.adapter.increment_one(
        plugin.model_name,
        [Where("id", provider["id"]), Where("domain", provider["domain"])],
        {},
        {"domainVerified": True},
    )
    if not verified:
