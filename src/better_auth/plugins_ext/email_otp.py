...
class EmailOTPPlugin(Plugin):
    ...
    async def _route_send_verification_otp(self, ctx: Ctx) -> AuthResponse:
        ...
        # middleware does.
        await validate_form_csrf(ctx.auth, ctx)
        body = ctx.body()
    ...
    async def _route_check_verification_otp(self, ctx: Ctx) -> AuthResponse:
        # TS d8327f1fe: verify the OTP itself BEFORE checking whether the account exists,
        # so a wrong/missing OTP yields the same INVALID_OTP regardless of registration —
        # the earlier user-existence check leaked enumeration via the error code.
        body = ctx.body()
        ...
        if await self._find_user(email) is None:
            # safe to leak existence: the caller already proved a valid OTP for this email
    ...
    async def _route_reset_password(self, ctx: Ctx) -> AuthResponse:
        ...
        email = body["email"].lower()
        # TS 2b4a14f18: validate the new password BEFORE consuming the OTP, so a
        # too-short/too-long password doesn't burn the (still-valid) OTP — the user
        # retries with the same code instead of having to request a fresh one.
