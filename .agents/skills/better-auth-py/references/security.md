...
`advanced.ipAddress` maps to `IPAddressOptions`. The resolved IP is the rate-limit key and the
...
`base_url` uses its `protocol` (`"https"` gives `True`, `"http"` gives `False`, `auto`/unset falls
back to `BETTER_AUTH_ENV`/`NODE_ENV` == `production`). Pass `use_secure_cookies=True` explicitly
